...
namespace System.Threading
{
    ...
    public struct SpinWait
    {
        ...
        public void SpinOnce(int sleep1Threshold)
        {
            ...
            {
                throw new ArgumentOutOfRangeException(nameof(sleep1Threshold), sleep1Threshold, Strings.ArgumentOutOfRange_NeedNonNegOrNegative1);
            }

            if (sleep1Threshold is >= 0 and < YieldThreshold)
            {
        ...
        public static bool SpinUntil(Func<bool> condition, int millisecondsTimeout)
        {
            if (millisecondsTimeout < Timeout.Infinite)
            {
                throw new ArgumentOutOfRangeException(
                   nameof(millisecondsTimeout), millisecondsTimeout, Strings.SpinWait_SpinUntil_TimeoutWrong);
            }
            if (condition == null)
            {
                throw new ArgumentNullException(nameof(condition), Strings.SpinWait_SpinUntil_ArgumentNull);
            }
            uint startTime = 0;
            if (millisecondsTimeout is not 0 and not Timeout.Infinite)
            {
